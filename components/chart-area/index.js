// components/chart-area/index.js
// Area chart - cumulative/volume trend. `data` is either a single series
// (`[{ x, y }]`) or multiple overlapping series (`[{ name, values: [{ x, y }] }]`).
...
class DeckChartArea extends DeckElement {
	static tag = 'deck-chart-area';
	...
	_draw() {
		...
		const svg = d3.select(this._root)
			...
			.attr('role', 'img')
			.attr('aria-label', this.getAttribute('label') || 'area chart');
		svg.append('title').text(this.getAttribute('label') || 'area chart');
		...
		g.selectAll('path.area').data(series).join('path')
			.attr('class', 'area')
...
customElements.define(DeckChartArea.tag, DeckChartArea);
